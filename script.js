...
async function main() {
    ...
    for (let i=0 ; i < liste.length ; i++) {
        ...
        const reserves_0_contract_ = await reserves_mute_contract.token0({ blockTag: blockNumber });
        const reserves_0_contract = new ethers.Contract(reserves_0_contract_, ERC20_abi, provider);
        const reserves_0_name = await reserves_0_contract.name({ blockTag: blockNumber });
        const reserves_0_symbol = await reserves_0_contract.symbol({ blockTag: blockNumber });
        const reserves_0_decimal = await reserves_0_contract.decimals({ blockTag: blockNumber });

        const reserves_1_contract_ = await reserves_mute_contract.token1({ blockTag: blockNumber });
        const reserves_1_contract = new ethers.Contract(reserves_1_contract_, ERC20_abi, provider);
        const reserves_1_name = await reserves_1_contract.name({ blockTag: blockNumber });
        const reserves_1_symbol = await reserves_1_contract.symbol({ blockTag: blockNumber });
        const reserves_1_decimal = await reserves_1_contract.decimals({ blockTag: blockNumber });
